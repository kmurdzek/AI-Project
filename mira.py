...
class MiraClassifier:
    ...
    def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, Cgrid):
        ...
        """
        # training on random indexes in the desired training data amount
        self.features = trainingData[0].keys()
        "*** YOUR CODE HERE ***"
            ...
            for i in range(self.percentageTrain):
                ...
                if(self.random == 1):
                    index = random.randint(
                        0, len(trainingData)-1)  # pick a random index in the training data, learn from that piece of data
                guess = self.classify([trainingData[index]])[0]
                if guess != trainingLabels[index]:  # training
                    t = min(
                        0.001, (self.weights[guess] - self.weights[trainingLabels[index]]) * trainingData[index] + 1.0 / 2.0 * (trainingData[index] * trainingData[index]))
                    # multiply each feauture by tau
                    for keys in self.features:
                        trainingData[index][keys] *= t
